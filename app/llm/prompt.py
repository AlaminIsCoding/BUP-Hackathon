...
The system prompt pins the exact ``directive_interpretation`` JSON shape and
carries few-shot examples for all six directive types, paraphrase handling,
relative percentages, time windows, distractors, and combined notes.
"""
...
Return ONLY a JSON array (no prose, no markdown fences). Emit exactly one object per operator note, in note order, matching this schema:
[
  {
...
Output: [{"note_index": 0, "applies": true, "directive_type": "solar_reduction", "structured_adjustment": {"hours": [11, 12], "factor": 0.2}, "explanation": "80% reduction leaves 20% usable solar in hours 11-12."}]

Note: "Use only half the solar forecast between noon and 3 PM."
Output: [{"note_index": 0, "applies": true, "directive_type": "solar_reduction", "structured_adjustment": {"hours": [12, 13, 14], "factor": 0.5}, "explanation": "Half the forecast means a usable factor of 0.5."}]

Note: "Keep 25% of the solar between 9 AM and 11 AM."
Output: [{"note_index": 0, "applies": true, "directive_type": "solar_reduction", "structured_adjustment": {"hours": [9, 10], "factor": 0.25}, "explanation": "Only a quarter of the solar forecast remains usable."}]

Note: "The data center needs at least 80 kWh in the battery from 6 PM until 10 PM."
Output: [{"note_index": 0, "applies": true, "directive_type": "minimum_battery_reserve", "structured_adjustment": {"hours": [18, 19, 20, 21], "minimum_energy_kwh": 80}, "explanation": "Backup reserve of 80 kWh required during the evening window."}]

Note: "Do not charge the battery from 2 AM to 4 AM."
Output: [{"note_index": 0, "applies": true, "directive_type": "no_charge_window", "structured_adjustment": {"hours": [2, 3]}, "explanation": "Charging is unavailable in hours 2-3."}]

Note: "No discharging between 7 PM and 9 PM."
Output: [{"note_index": 0, "applies": true, "directive_type": "no_discharge_window", "structured_adjustment": {"hours": [19, 20]}, "explanation": "Discharging is unavailable in hours 19-20."}]

Note: "Grid intake must stay at or below 190 kWh from 7 PM until 10 PM while the substation is constrained."
Output: [{"note_index": 0, "applies": true, "directive_type": "max_grid_window", "structured_adjustment": {"hours": [19, 20, 21], "max_grid_kwh": 190}, "explanation": "Grid import is capped at 190 kWh during the constraint."}]

Note: "A seminar room booking was moved to next week."
Output: [{"note_index": 0, "applies": false, "directive_type": "no_op", "structured_adjustment": null, "explanation": "This note does not affect today's energy schedule."}]
...
def build_messages(notes: list[str], battery: BatterySpec) -> list[dict[str, str]]:
    ...
    lines.append("Return the JSON array only.")
    return [
